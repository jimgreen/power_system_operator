...
> 2026-08-26 运行状态二态化最终修订（最高优先级）：`operator_control.oper_status` 只允许 `0=停止`、`1=运行`，彻底删除暂停状态。SQLAlchemy 新库必须带 `CHECK` 约束，既有 SQLite 库初始化时把旧值 2 及其他非法值迁移为 0，并安装插入/更新保护，后续数据库写入非 0/1 必须失败。MMI 运行状态选择只含停止/运行，人工运行按钮只保留“启动”“停止”，不得生成 `pauseButton`、“启动 / 继续”或任何写入状态 2 的代码。Core/IO 只按停止和运行判断。模拟器通信协议顶层 `simu_status=2` 仍表示外部模拟器自身暂停，不属于 EMS 本地 `oper_status`，不得误删该协议兼容能力。本文后续所有关于 EMS 本地“暂停/继续”、`OPER_PAUSED=2` 或暂停按钮的旧表述均失效。

> 2026-08-26 独立进程架构最终修订（最高优先级）：`operator_mmi.py`、`operator_core.py`、`operator_io.py` 必须始终是三个相互独立的操作系统进程。MMI 保持单实例，但只负责按 PID、创建时间、Python 解释器、入口脚本、数据库/配置路径精确附着、监视和显式启动/停止/重启 Core、IO；缺失时启动顺序为 Core → IO，关闭 MMI 不得停止 Core/IO。Core 和 IO 必须能够完全脱离 MMI 独立启动和持续运行。MMI 在运行控制区下方提供高度不超过约 98 像素的两行紧凑进程表：第一行 Core、第二行 IO；每行显示运行/停止/异常、PID、墙钟启动时间和启动/停止/重启按钮，入口、数据库与解释器可放工具提示。进程状态不得与电网模拟器业务连接状态混为一谈。本文后续任何“MMI 内嵌 Core/IO 子线程”“关闭 MMI 自动停止后台服务”“默认模式不得存在独立 Core/IO 进程”的旧表述均已失效，必须按本修订解释或改写；SQLite 并发应按三个独立进程各自持有 Engine/短 Session、WAL、busy_timeout、短事务和有界重试实现。

> 2026-08-25 EMS 原理与点位文档更新：除综合技术与用户操作手册外，新增独立交付物 `power_system_operator_ems_principles_and_scada_points.html` 及可重复生成脚本 `generate_ems_principles_html.py`。文档必须基于当前 Core/IO/策略实现说明 EMS 功能、线程与数据库架构、双周期/三类时钟、量测映射、风光理论出力、新能源优先调度、储能边界、YT/YK 闭环、历史审计、SQLite 并发和任务恢复；四遥点表必须从指定 `ems.db` 只读提取实际 YC/YX/YT/YK 定义和值/时标，禁止手抄或硬编码。成品为无外部依赖的中文 UTF-8 单文件 HTML，支持搜索/类型过滤、响应式布局和 A4 打印，并经过真实浏览器桌面/窄屏验收。

> 2026-08-25 决策日志与历史刷新更新：运行日志页把控制决策的 `process` 从 `[1]...[N]` 树节点改为独立的只读逐行中文日志区，每行固定展示步骤号、已执行/未触发、中文步骤名、三位小数关键输入输出和中文原因；下方树只保留触发条件、输入、输出、平衡校验/警告，原始 JSON 继续完整保留。运行日志表格不得由 1 秒或 `data_period` 定时器自动查询，只能在 MMI 初始化、切换进入日志页、查询、重置、分页或修改每页条数时按需刷新。系统主页曲线继续每 1 秒自动刷新；历史曲线页继续按 `data_period` 自动查询并重绘已勾选曲线，但周期刷新不得重建左侧四遥点树。

> 2026-08-25 稳定客户端身份更新：所有发往真实 `simulator_io` 的 `action=read/write/ping` 请求都必须在顶层携带固定 `client_id="power_system_operator"`。该身份由程序常量强制写入，调用者不得用任意字符串覆盖；短连接、持续连接、断线重连和心跳均保持同一身份，使模拟器按最近 10 秒成功 read/write 的业务活跃窗口稳定统计一个 EMS/operator 客户端，而不是按瞬时套接字或累计建连次数统计。

> 2026-08-25 MMI 单实例可见性自愈更新：单实例锁持有者必须原子发布真实 PID、解释器、启动参数和工作目录。首实例必须在 Qt `show()` 后再次执行正常显示、原生 `SW_RESTORE` 和事件循环启动后的延迟恢复，避免 `Start-Process -WindowStyle Hidden` 或等价启动标志留下“窗口不可见但 Python 宿主仍持锁”的假象。第二次启动必须读取并显示真实持有者 PID，优先恢复、置前已有 Qt 窗口；恢复成功时以退出码 0 结束，只有锁仍被占用但窗口无法恢复时才以退出码 2 结束并给出明确诊断。持有者正常退出时删除元数据，异常退出后的旧元数据由下一个成功持锁者原子覆盖。正式启动禁止对 MMI 使用隐藏窗口参数，推荐使用 `pythonw.exe` 或等价无控制台桌面方式。
...
> 2026-08-24 发布规范更新：补充“保存修改、提交远方、重启服务”的完整执行边界，要求显式暂存清单、测试后提交、四方 Git SHA 一致性、按完整入口脚本和数据库路径精确重启 MMI、Core、IO 三个独立进程、保护外部电网模拟器，并以三个新进程的 PID/创建时间、端口归属、数据库、RTU 墙钟、运行日志和真实 MMI 窗口作为发布后健康证据。

> 2026-08-26 进程架构更新：默认运行形态为 `operator_mmi`、`operator_core`、`operator_io` 三个独立进程；MMI 启动时附着已有 Core/IO，缺失时按 Core → IO 启动，普通关闭 MMI 不停止后台进程。模拟器新任务或时钟回退恢复只精确停启属于当前数据库和入口的 Core 进程。Core/IO 均是正式常驻入口，也可脱离 MMI 独立运行。

> 2026-08-24 MMI 单实例更新：`operator_mmi` 必须通过操作系统级原子单实例锁保证同一台主机、同一用户会话内最多只有一个界面进程。Windows 使用命名 Mutex，其他平台使用等价的进程锁；禁止用存在竞态的进程扫描代替。入口必须在初始化 `ems.db`、创建 Qt 窗口或管理 Core/IO 进程之前获取锁；第二次启动必须输出真实持有者 PID、尝试恢复现有窗口，恢复成功时正常结束，恢复失败时非零结束，不得触碰数据库或启动重复后台进程。锁随正常退出显式释放，进程异常退出时由操作系统自动释放，不能遗留永久假锁；诊断元数据只用于定位持有者，不能取代操作系统锁。

> 2026-08-24 运行日志分页更新：`operator_log` 继续永久保存且不设置记录总量上限，但 MMI 禁止一次性查询并渲染全部日志。日志页必须先对当前过滤条件执行总数查询，再使用数据库 `LIMIT/OFFSET` 分页读取；默认每页 100 条，可切换 50/100/200/500 条，提供总条数、当前页/总页数、上一页和下一页。查询或重置条件时回到第一页；日志页不参加任何周期自动刷新，显式按需刷新时仍应尽量保留当前页和已选日志，避免页面堵塞或用户阅读位置丢失。

> 2026-08-25 参数编辑保护与曲线游标更新：顶部开闭环/双周期、设备静态参数及 RTU/四遥定义一旦被人工修改，控件或单元格必须立即使用醒目的橙黄色脏状态提示；保存或用户点击对应区域的“手动刷新参数”并确认放弃修改前，1 秒刷新、`data_period` 周期刷新和切页刷新均不得覆盖未保存值。运行状态、双时钟、连接状态等只读实时量继续刷新，运行/停止动作只能写 `oper_status`，不得顺带保存待提交参数。所有使用 `InteractivePlot` 的曲线板（当前包括系统主页和历史曲线）都必须让游标文字框跟随真实鼠标坐标移动；即使仍吸附同一个数据时刻，只要鼠标位置变化，文字框也必须同步移动。文字框接近绘图区边界时自动翻转并始终保持在绘图区内，背景颜色 alpha 必须严格等于 0，禁止使用实色或半透明填充，仅保留可读文字、曲线色标和轻量边框。
...
> 2026-08-24 模拟器任务同步更新：YC/YX 成功响应携带 `run_seq`、`simu_time_start`、`simu_status`、`runtime_ready`。EMS 以 `run_seq` 变化识别新的模拟任务，不再只依赖运行时刻回退；新任务首个有效断面就绪前必须停止 Core 调度，完成运行数据清理和首断面同步后再恢复 Core。项目 Mock 同步实现任务元数据和旧任务命令拒绝；完全没有任务元数据的旧对端继续使用原时钟回退兼容逻辑。
...
交付一个由 MMI、Core、IO 三个独立操作系统进程组成的完整工程：

1. `operator_mmi`：唯一 PyQt6 桌面界面进程。入口使用操作系统级原子单实例锁；启动后附着或启动独立 Core、IO，显示两行紧凑进程表和 PID/启动时间，提供分别启停/重启；关闭 MMI 不停止后台进程。界面支持运行控制、开环/闭环、双周期、双时钟、主页曲线、设备定义、四遥定义、分页运行日志、历史曲线，以及建立/中断电网模拟器连接。运行日志必须通过数据库分页查询，并以独立逐行中文日志展示每次控制决策的真实策略过程，同时保留触发条件、完整输入、完整输出、平衡校验和原始 JSON。

2. Core 进程：使用进程私有 SQLAlchemy `Database/Engine` 访问同一个 SQLite 文件，每 0.5 秒检查运行控制状态，根据数据时钟执行数据处理，根据决策时钟执行新能源优先策略；`operator_core.py` 是正式独立常驻入口。

3. IO 进程：使用进程私有 SQLAlchemy `Database/Engine`，默认作为 TCP 客户端访问外部 `simulator_io`，按数据周期获取 YC/YX，每 1 秒发送变化的 YT/YK；任务切换或时钟回退时通过严格进程身份校验停启独立 Core。`operator_io.py` 是正式独立常驻入口，并保留显式 TCP Server 兼容模式。
...
8. EMS 原理与四遥点位参考文档：生成 `power_system_operator_ems_principles_and_scada_points.html`，并提供 `generate_ems_principles_html.py --db <数据库> --output <HTML>` 重复生成入口。该脚本必须以 SQLite `mode=ro` 方式读取指定数据库，执行完整性检查，按点号提取 YC/YX/YT/YK 实际点位，不能修改数据库。HTML 必须完整说明当前实现的功能原理和算法流程，并展示点号、点名、方向、对象域、业务含义、单位/编码、三位小数当前值、`HH:mm:ss` 运行时标和 `time>0` 有效性；页面支持关键词搜索、四遥类型过滤、响应式布局和 A4 打印，不依赖 CDN、远程图片、字体或 Web 服务。
...
- MMI、Core、IO 三个独立进程的业务运行不依赖 Web 浏览器或 Web 服务。HTML 手册只作为离线文档交付和验收对象，不得被引入业务运行依赖。
...
- 最新强制口径：储能模型、新建库、示例数据、导入目标、Core 和 MMI 均不得出现 `soc_init`。旧库只删除该列并原样保留 `soc_curr`；停止→运行、IO 断线、Core 重启和模拟器时钟回退都不得复位 `soc_curr`。只有能够唯一映射到设备且 `time > 0` 的实时 SOC YC 才能更新 `soc_curr`。
...
power_system_operator_ems_principles_and_scada_points.html
generate_ems_principles_html.py
init_db.py
...
|---|---|---|
| `id` | int 主键 | 固定 1 |
| `oper_status` | int | 初值 0；只允许 0=停止、1=运行；旧值 2/非法值迁移为 0，数据库拒绝后续非法写入 |
...
| `operator_core` 独立常驻进程 | 禁止 | 禁止 | 仅限本地已有点 |
| `operator_io` 独立常驻进程 / 兼容 Server | 禁止 | 禁止 | 仅限本地已有点 |
...
- Core 始终以独立常驻进程运行，循环必须支持正常终止并用可中断等待替代不可控的永久 `sleep`；进程创建自己的 `Database/Engine`，不使用 MMI Session。`operator_core.py` 使用按数据库绝对路径生成的 PID 元数据并保留 `--poll`、`--pid-file` 和 `--once`，MMI 只通过精确进程身份管理它。
...
模拟器运行时刻可能在一个数据采集周期内跨过 60 秒或更多；不能因此按跨过的运行秒数补执行多个决策，也不能每个数据断面都绕过墙钟周期。每个 Core `tick` 最多执行一次决策，并使用当时最新的 `data_time_curr` 作为该次决策时刻。只有决策事务成功提交后才更新进程内的上次成功决策单调时刻；失败允许后续轮询重试。停止或停止后重新启动时清除进程内调度基准，恢复运行后仍按“首次有待决策的新断面立即执行，后续按墙钟周期执行”的规则重新计时。
...
11. MMI 不得把 `process` 继续渲染成需要逐层展开的 `[1]...[N]` 树。格式化详情页顶部必须提供独立只读多行日志区，严格按 `step` 顺序一行展示一个步骤，格式为“步骤号｜已执行/未触发｜中文步骤名称｜关键输入输出摘要｜原因：中文原因”；长行按控件宽度换行并支持纵向滚动。格式化树只保留触发条件、输入、输出、平衡校验/警告，完整 `process` 仍保留在原始 JSON 页签中。未知步骤必须有安全的通用降级摘要，不能导致详情页崩溃。
...
```text
operator_io 独立进程 --TCP client--> simulator_io
```

采用 UTF-8 JSON Lines：一条 TCP 连接发送一行 JSON 请求并接收一行 JSON 响应。限制单行最大 2 MiB，设置连接超时，检查 JSON 类型和 `ok` 状态。每个 read/write/ping 顶层都必须携带固定 `client_id="power_system_operator"`；重连不得改变该身份，调用者提供的其他 ID 必须被固定身份覆盖。
...
- YC/YX/YT/YK 的 `pnt_no`、`name` 和其他定义字段在运行期只读。Core/IO 独立进程和其他后台工作单元只能修改本地已有点的 `value` 和 `time`；严禁根据通信报文创建点位、覆盖 `name` 或修改 `pnt_no`。本地 `pnt_no` 是运行期更新的唯一身份键，报文 `name` 不参与写库；点位创建和定义修改只能由显式建库、定义导入或 MMI 四遥定义操作完成。
...
```text
精确停止并确认当前 Core 独立进程退出
→ 本地数据时钟和控制时钟回退到 0
→ 清理历史、日志、四遥实时值/时刻和 Bridge 周期状态
→ 保存任务元数据并应用已就绪的首个有效 YC/YX 断面和权威时刻
→ 提交事务
→ 仅在首个有效断面就绪后重新启动 Core 独立进程
```

具体要求：

1. IO 独立进程使用 `CoreProcessManager` 管理 Core，只能按 PID、创建时间、解释器、入口脚本和数据库绝对路径精确停止、启动属于本工程的 Core，禁止按进程名扫描或终止其他 Python 进程；MMI 与 IO 使用同一套身份校验规则，任何时刻最多只有一个匹配当前数据库的 Core。

2. 进程管理器必须持有明确的 PID、创建时间、解释器、入口脚本、数据库绝对路径和有界停止超时；附着或停止前必须重新核验身份并防止 PID 复用。停止失败或退出超时时立即报错，禁止修改本地时钟、历史、日志和四遥数据。

3. Core 停止与重启必须在 SQLite 写事务之外；停止后必须等待并确认旧进程已退出，才能进入清理事务。
...
系统停止、`io_connect_enabled=0` 或 Bridge 正常退出时，也要把对应 RTU 状态写为 0。Bridge 再次成功交换后恢复为 1。禁止仅因 `io_connect_enabled=1` 就把 RTU 状态写为 1。
...
保存参数一次写入 `control_status`、`data_period`、`oper_period`。用户修改任一参数控件后，该控件使用橙黄色背景和边框标记未保存状态，保存按钮启用；固定 1 秒刷新仍更新运行状态、双时钟和连接状态，但不得覆盖这些脏参数。状态组合框以及“启动”“停止”按钮只写 `oper_status`，绝不能顺带写入或保存待提交的控制参数。“手动刷新参数”在存在未保存修改时先确认是否放弃，确认后才从数据库重载并清除脏状态；取消时必须原样保留用户输入。界面使用两个相互独立的 `QTimer`：第一个固定每 1 秒刷新顶部控制状态、双时钟、连接状态和系统主页；第二个以数据库已保存的 `max(1, data_period) * 1000` 毫秒为周期，只刷新当前的设备定义、RTU / 四遥定义或历史曲线页。运行日志明确不接入任何定时器，只允许按需查询。数据周期被外部进程或界面保存后，页面定时器必须在下一次 1 秒状态刷新时同步新间隔。`oper_period` 只驱动控制决策，绝不能用于界面刷新定时器。

切换任一主页面时必须立即刷新目标页面一次，不等待页面数据周期；进入设备定义、RTU / 四遥定义或历史曲线后，从切换时刻重新启动页面周期计时。进入运行日志页时停止页面定时器并只执行一次分页查询；进入系统主页后同样停止页面定时器，由固定 1 秒定时器更新主页。
...
4. 运行日志：
   ...
   - 对 `log_type=LOG_DECISION` 的记录，必须解析 `log_info` JSON。格式化详情页顶部使用标题为“决策过程（逐行日志）”的只读 `QPlainTextEdit`，按 `step` 顺序一行显示一个策略步骤，格式为“`NN｜已执行/未触发｜中文步骤名｜关键输入输出｜原因：中文原因`”；功率等浮点数显示三位小数，长行按控件宽度自动换行并支持纵向滚动。下方树只展示“触发条件、输入、输出、平衡校验/警告”四组，不得再出现 `[1]...[N]` 决策过程树节点。输入和输出中的设备列表继续使用树形列表；用户无需阅读原始 JSON 即可了解完整策略过程。
   ...
   - 当前停留在本页时不得按固定 1 秒、`data_period` 或任何其他定时器自动刷新。只允许在 MMI 初始化、切换进入本页、点击查询/重置、上一页/下一页或修改每页条数时执行分页查询；这些显式刷新应保留适用的过滤条件、已选日志、详情页签和滚动位置，不能无故跳到最新记录。
...
README 必须说明：先启动 Mock，再启动 MMI；MMI 自动附着或按 Core → IO 启动两个独立后台进程，关闭 MMI 不停止 Core/IO。最后在 MMI 选择控制模式、保存参数并点击“启动”。
...
13.2. 分别验证停止→运行、IO 暂时断线和模拟器时钟回退：在没有有效 SOC YC 时 `soc_curr` 始终保持原值；收到 `<设备名称>.当前SOC`、`<设备名称>.SOC`、`<设备名称>.当前荷电状态` 或兼容 `dev_estore.<id>.soc_curr` 且 `time > 0` 的 YC 后才更新，多个有效 SOC YC 采用时刻最新的一条，`time <= 0` 不得生效。

14. 数据库拒绝 `oper_status` 取 0/1 之外的值，旧暂停值初始化时迁移为停止。
...
31. MMI 固定 1 秒定时器只刷新顶部状态和系统主页；设备、RTU / 四遥、历史曲线使用独立定时器并严格跟随 `data_period`，即使 `oper_period` 不同也不得误用。运行日志不接入任一定时器，停留在日志页时反复触发 1 秒或页面定时器都不得新增数据库查询。切换五个主页面时目标页面都立即刷新，不能等待周期；进入日志页时查询一次并保持页面定时器停止。
...
34. 运行日志页可以按类型、运行时刻和关键词查询；先执行匹配总数查询，再用 `LIMIT/OFFSET` 分页读取，默认每页 100 条且可选 50/100/200/500，页面显示总条数、当前页/总页数并正确控制上一页/下一页。至少插入 1205 条日志，证明首页只查询和渲染 100 条、共有 13 页，而数据库全部 1205 条仍可通过翻页访问；禁止全量 `.all()` 后内存切片。选择 `LOG_DECISION` 后，独立多行日志区按步骤显示“编号、已执行/未触发、中文步骤名、三位小数关键输入输出、中文原因”，树中只保留“触发条件、输入、输出、平衡校验/警告”四组且不存在 `[1]...[N]` 过程节点；原始 JSON 可以切换查看，非 JSON 旧日志不会导致界面异常。

35. 运行日志页中的墙钟、运行时刻和浮点数格式正确；日志页停留期间固定 1 秒和 `data_period` 定时器均不触发日志查询。进入页面、查询、重置、分页或修改每页条数时才按需刷新，并保留适用的过滤条件、选中决策、详情页签和滚动位置，不会把用户正在查阅的记录强制切换为最新日志。
...
- 手册至少内嵌以下 6 张 1480×920 PNG 截图，每张都要有准确的中文 `alt` 和编号图注：
  ...
  1. 系统主页与顶部运行控制区：同时显示运行/停止、开闭环、数据/决策周期、数据/控制时刻、保存参数、手动刷新参数、建立/中断连接、当前断面、曲线树和系统曲线。
  ...
  5. 控制决策运行日志：左侧至少有一条真实 `LOG_DECISION`，右侧顶部显示“决策过程（逐行日志）”，每个策略步骤一行；下方树显示触发条件、输入、输出、平衡校验/警告，并保留原始记录页签。
...
1. **手册概览**：说明系统用途、适用对象、MMI/Core/IO 三个独立进程、外部模拟器接口、正式库与示例库边界，以及本手册对应的软件版本。

2. **快速开始**：给出 Python 版本检查、环境创建、依赖安装、`pyuic6`、正式库初始化、独立示例库、Mock 联调以及启动 MMI/Core/IO 的可复制 PowerShell 命令；说明 MMI 附着已有进程、按 Core → IO 补启缺失进程及首次运行检查点，并说明关闭 MMI 不停止后台进程。

3. **系统架构**：使用内联 SVG 展示外部 `simulator_io`、MMI、IO/Core 独立进程和共享 `ems.db`，并分别解释量测数据流、控制命令流、进程生命周期和数据库访问关系。

4. **进程职责与调度**：说明 Core 的 0.5 秒轮询、数据周期与决策周期的单调墙钟调度、IO 的读取/写入周期、MMI 页面刷新周期、独立进程的附着/启停/重启，以及业务“停止”和关闭 MMI 进程的区别。

5. **安装、初始化与部署**：说明依赖、目录、数据库初始化/迁移、正式库不得写示例数据、旧 `power.db` 只读导入、配置参数、日志，以及 MMI/Core/IO 三个独立进程的生命周期管理。
...
10. **MMI 使用说明**：只描述当前存在的系统主页、设备定义、四遥定义、运行日志、历史曲线五个主页面，不得加入“曲线定义”；结合至少 6 张真实界面截图，逐步说明启动/停止、开环/闭环、双周期、双时钟、Core/IO 两行进程表与分别启停/重启、建立/中断连接、主页监视、设备参数保存、RTU/四遥核对、决策日志审计、历史多点查询、编辑保护、页面切换刷新、三位小数和曲线游标。

11. **标准操作流程**：分别给出首次部署、正常运行、开闭环切换、连接中断恢复、停止后重新运行和模拟器时钟回退的操作步骤、预期状态和注意事项。

12. **运行日志与历史数据**：说明 `LOG_DECISION` 的触发条件、输入、策略步骤、输出和平衡校验结构；说明历史断面和四遥历史的写入时机、查询方式以及“无自动存储/查询条数上限但必须监控磁盘”的运维要求。

13. **SQLite 多进程并发可靠性**：说明 MMI/Core/IO 每个独立进程各自持有 Engine/短 Session，并使用 WAL、`busy_timeout`、外键、短事务、写重试、事务边界、网络调用期间不持有事务和 SQLite 单写者事实。
...
- 明确系统共有 17 张业务表、5 个 MMI 主页面，以及 MMI、Core、IO 三个正式独立进程；`simulator_io_mock` 是外部联调工具。不得出现 `curve_def`、`soc_init`、偏航角字段、偏航角设定 YT 或桨距角设定 YT。

- 明确停止切到运行会清空历史、日志及实时四遥的 `value/time`，但不会复位储能 `soc_curr`。逻辑停止只改变业务状态，不等价于杀死进程；数据库状态只允许停止和运行。
...
#### 15.6 EMS 功能原理、算法流程与四遥点位 HTML

除上述综合手册外，必须生成仓库根目录文件 `power_system_operator_ems_principles_and_scada_points.html` 和脚本 `generate_ems_principles_html.py`：

- 生成脚本提供 `--db` 和 `--output` 参数；默认从仓库根目录 `ems.db` 只读提取数据，SQLite URI 必须使用 `mode=ro`，生成前执行 `PRAGMA integrity_check`，不得创建、修改或迁移数据库。
- HTML 至少包含“功能定位、系统架构、周期与时间语义、数据处理流程、新能源优先算法、YT/YK 生成与闭环、历史与审计、并发与异常恢复、四遥点位总表、运行核对清单”10 个可导航章节。
- 原理和公式必须与最终 `power_operator/core.py`、`strategy.py`、`wind_power.py`、`io_service.py` 一致，明确风机分段三次曲线、光伏辐照折算、储能 SOC/效率功率上限、开环固定贡献、富余/缺额分支、功率平衡、YT 每轮刷新时标和 YK 状态差异边界。
- YC/YX/YT/YK 点表必须由数据库查询结果生成并按点号排序，分别报告点数；每行至少展示点号、点名、数据方向、对象域、业务含义、单位或状态编码、当前值、`HH:mm:ss` 时标及 `time>0` 有效性。当前值和时标必须声明为生成时断面，不得伪装成固定定义值。
- HTML 为单文件中文 UTF-8，不使用外部脚本、样式、图片或字体；支持四遥类型按钮和关键词搜索过滤，显示当前可见行数，包含响应式桌面/窄屏样式和 A4 `@media print`。
- 增加自动化测试，使用临时数据库证明四张点表来自实际查询、中文和特殊字符正确转义、浮点数三位小数、运行时标正确、`time=0` 显示无效、模板无未替换标记且不存在外部 HTTP 资源。
- 静态验收至少检查 UTF-8、DOCTYPE、唯一锚点、10 个章节、4 张点表、数据库点数与 HTML 数据行数一致、无模板残留和无外部资源。真实浏览器分别在约 1440×900 和 390×844 视口打开，验证目录导航、搜索、类型过滤、表格局部横向滚动、页面无整体横向溢出、A4 样式存在且控制台错误为 0；至少保存一张桌面截图作为验收证据。
...
2. 启动 Mock，再启动 MMI；证明 MMI 自动附着或按 Core → IO 启动两个独立进程，进程表显示两个不同 PID 和墙钟启动时间。关闭 MMI 后证明 Core/IO 仍持续运行，随后用各自正式入口验证它们可脱离 MMI 独立启动。
...
6. 联调结束后正常关闭本次 MMI，证明 Core、IO 两个独立进程不随界面退出；如需结束联调服务，必须通过精确身份校验分别停止 IO、Core，不得误停其他 Python 进程。
...
1. 通过完整脚本绝对路径、数据库绝对路径、解释器和创建时间分别识别唯一的 `operator_mmi.py`、`operator_core.py`、`operator_io.py` 进程；已有匹配 Core/IO 应被 MMI 附着，不得当作旧架构清除。只处理精确归属于当前工程和数据库的重复或待重启进程，不得只按 `python.exe`、模糊名称、父进程或窗口标题结束进程，不得批量杀死其他 Python 服务。
...
- 关闭 MMI 后必须确认界面 PID 已退出，同时确认 Core/IO 两个独立进程仍保持运行。启动时使用 `pythonw.exe` 或等价无控制台桌面方式保持 MMI 窗口可见，并传入数据库、Bridge 主机、端口、RTU 和轮询参数；禁止对 MMI 使用 `Start-Process -WindowStyle Hidden`。即使外部启动器误传隐藏标志，程序也必须通过二次 Qt 显示、Windows `SW_RESTORE` 和延迟恢复使窗口可见。重复启动恢复现有窗口时返回 0，未找到可恢复窗口时才返回 2。Core/IO 使用各自 PID 元数据，并在 MMI 两行进程表中显示真实 PID 和启动时间。

- MMI、Core、IO 应各自使用带时刻或发布标识的 `.runtime` 标准输出/错误日志，明确记录进程 PID、创建时间、启动、停止和异常。启动后必须有合理的稳定等待窗口，不得把 `Start-Process` 返回 PID 或短暂存活当作完整健康证据。

发布后至少完成以下验收：

1. 重新列出唯一 MMI 的完整命令行，报告旧 PID → 新 PID；分别列出唯一 Core、IO 独立进程的 PID、创建时间、解释器、入口与数据库参数。验证 MMI 能附着已有后台进程，关闭/重开 MMI 不改变 Core/IO PID。首实例运行时再次执行同一 MMI 启动命令，验证第二实例在初始化数据库和进程管理器之前被单实例锁拒绝、输出真实持有者 PID、恢复并置前已有窗口、以 0 正常退出，且首实例 PID、窗口、Core/IO 和 RTU 墙钟均不受影响；若锁被占用但没有可恢复窗口则必须以 2 退出并明确诊断。还要故意以隐藏窗口参数启动一次，验证 MMI 能自动恢复为可见。首实例正常或异常退出后必须验证锁可重新获取，持有者诊断元数据能够清理或被新持有者覆盖。
...
6. 最后再次检查 `git status --short --branch` 和 `git status --porcelain`；如工作区存在用户的预有修改，必须列出并说明它们未被提交。只有提交范围、远端 SHA、MMI/Core/IO 三个独立进程、外部模拟器、TCP、数据库、日志和窗口全部得到真实证据后，才能报告发布完成。
...
- 不得定义、创建、显示、计算、导入、更新、发送或回写 `angle_yaw_curr`、风机偏航角设定 YT、风机桨距角设定 YT 和 `soc_init`；不得生成 `<设备名称>.当前偏航角`、`dev_wind_gen.<id>.angle_yaw_curr` 或本提示词列出的六种禁用风机角度 YT 点位。旧库升级、旧库导入和 IO 数据包处理均不得使这些废弃定义重新出现。不得误删或禁用设备字段 `angle_pitch_curr`、YC“当前桨距角”及有功设定。不得在启动、连接中断或时钟回退时复位 `soc_curr`；SOC 只接受有效 YC 更新。
...
- 不得省略 `power_system_operator_ems_principles_and_scada_points.html` 或把四遥点位手工写死在文档中；必须保留可重复生成脚本，并证明 HTML 点数和当前指定数据库四张四遥表一致。
...
5. 独立 MMI 开环与闭环联调数据，以及 Core/IO 独立进程附着/自动启动、时钟回退时精确重启 Core、关闭 MMI 后后台进程保持运行的证据。
...
10. EMS 功能原理与四遥点位 HTML 的绝对路径、生成命令、数据源数据库、四遥分类点数和总点数、数据库完整性、静态结构/UTF-8/锚点/表格/外部资源检查、关键词与类型过滤交互、桌面和窄屏真实浏览器结果、控制台错误数量、截图路径和 SHA-256。

11. 若本次任务包含版本发布和服务重启：显式暂存清单、完整提交 SHA 和提交信息、四方 Git SHA 一致性、MMI/Core/IO 三个进程的旧/新 PID、创建时间与完整参数、各自启动停止顺序和单实例证据、外部模拟器和 `9001` 端口保护结果、数据库/RTU 墙钟新鲜度、本次进程日志、唯一 MMI 窗口以及最终工作区状态。
