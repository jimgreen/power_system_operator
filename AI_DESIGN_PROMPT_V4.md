...
YX 更新 `status`，且只允许使用 `time > 0` 的实时遥信。除兼容 `设备表名.设备ID.status` 外，还必须识别系统预定义点名 `<设备名称>.运行状态`；若 YX 描述不同，可用同点号的预定义 YK `<设备名称>.启停命令` 识别设备，但绝不能读取 YK 的目标值作为实际状态。YX 值为 0 时写入设备 `status=0`，非 0 时写入 `status=1`。多个有效 YX 映射到同一设备时使用 `time` 最新的一条。
...
输入只包含状态为运行的设备。负荷取所有运行负荷的非负当前功率合计。
...
高度尽量小，但不能层叠。采用两行紧凑网格加右侧 2×2 按钮：
...
顶部中间空白区域必须增加始终可见的“电网模拟器连接”状态卡，与控制区保持同一行且不遮挡按钮：任一 `scada_rtu.status=1` 时显示绿色“正常”，没有 RTU 或全部状态为 0 时显示红色“中断”。该卡片与 RTU / 四遥定义页的连接状态必须复用同一状态源，并且无论用户当前打开哪个页面都每 1 秒刷新。
...
保存参数一次写入 `control_status`、`data_period`、`oper_period`。状态按钮写入 `oper_status`。界面使用两个相互独立的 `QTimer`：第一个固定每 1 秒刷新顶部控制状态、双时钟、连接状态和系统主页；第二个以 `max(1, data_period) * 1000` 毫秒为周期，只刷新当前的设备定义、RTU / 四遥定义、运行日志或历史曲线页。数据周期被外部进程或界面修改后，页面定时器必须在下一次 1 秒状态刷新时同步新间隔。`oper_period` 只驱动控制决策，绝不能用于界面刷新定时器。
...
2. 设备定义：
   ...
   - 页面顶部设置紧凑的编辑说明和“保存设备修改”按钮。静态参数单元格使用浅黄色背景，双击、单击已选单元格或按编辑键均可进入编辑；已有设备 ID 和实时字段使用浅灰色背景并保持只读。保存按钮只有在设备表存在未保存修改时才启用。
   ...
   - 只要任一设备表格正在编辑或已经产生未保存修改，切页刷新和周期刷新都必须跳过设备表重载，不能覆盖用户输入；保存成功并重新加载后清除脏状态。

3. RTU / 四遥定义：

   - 页面顶部必须有紧凑的 `operator_io 连接状态` 卡片。

   - `scada_rtu.status=1` 时显示绿色“连接成功”；没有 RTU、状态为 0、交换失败、停止或暂停时显示红色“连接中断”。

   - 同时显示当前 RTU ID、对端 IP:端口、最后一次成功刷新时刻和在线 RTU 数量；这里的 RTU 最后刷新时刻必须按墙钟 `yyyy-MM-dd HH:mm:ss` 显示。

   - 页面顶部连接状态仍每 1 秒刷新；RTU 和四遥表格只按 `data_period` 自动刷新，切换进入本页时立即刷新。

   - 只要任一 RTU / 四遥表格正在编辑或已经产生未保存修改，切页刷新和周期刷新都必须跳过表格重载，不能覆盖用户输入；保存成功并重新加载后清除脏状态。
...
4. 运行日志：
   ...
   - 主区域使用可调 `QSplitter`：左侧或上部为全部匹配的 `operator_log` 列表，不得设置记录条数上限，至少显示墙钟时刻、运行时刻、类型、决策 ID 和摘要；右侧或下部为选中日志的详情区。
...
- `InteractivePlot` 至少支持多条曲线、图例、网格、时间横轴、滚轮缩放、左键平移、双击复位和鼠标数据游标。游标必须吸附到离鼠标最近的可见数据时刻，绘制竖向虚线并在所有已显示曲线上标记对应数据点；浮层以 `HH:mm:ss` 显示游标时刻，并以三位小数列出每条曲线的值。鼠标离开绘图区、数据源改变、缩放或复位时必须清除旧游标，不能显示过期数据。
...
26. MMI 正确显示开闭环、双周期、数据时刻、控制时刻、顶部电网模拟器“正常/中断”和 `operator_io` 连接成功/连接中断状态；两个时刻都必须由整数秒格式化为 `HH:mm:ss`，两个连接状态必须来自同一 RTU 状态源。
...
32. 设备或四遥表格存在活动编辑器或未保存修改时，切页和周期刷新均不覆盖单元格；保存后恢复刷新。历史曲线周期刷新不重建点树，页面切换或点名改变后已勾选点仍按表名和点号保持勾选。

33. 表格、只读文本框、曲线数据和纵轴刻度中的浮点数统一为三位小数；测试至少验证 `1.23456 -> 1.235`，并证明数据库值没有被显示格式化反写。

34. 运行日志页可以按类型、运行时刻和关键词查询；选择 `LOG_DECISION` 后正确展示“触发条件、输入、决策过程、输出、平衡校验/警告”五个分组，策略步骤顺序和执行/跳过原因完整，原始 JSON 可以切换查看，非 JSON 旧日志不会导致界面异常。

35. 运行日志页中的墙钟、运行时刻和浮点数格式正确；按 `data_period` 刷新时保留过滤条件、选中决策、详情分组、原始记录开关和滚动位置，且不会把用户正在查阅的记录强制切换为最新日志。

36. 系统主页和历史曲线均支持数据游标；移动鼠标后游标吸附最近的可见数据时刻，所有已显示曲线同时出现点标记，浮层完整显示时刻和所有曲线的三位小数值；离开、换数据、缩放和复位后旧游标消失。
...
- 手册至少内嵌以下 6 张 1480×920 PNG 截图，每张都要有准确的中文 `alt` 和编号图注：
  ...
  1. 系统主页与顶部运行控制区：同时显示运行/暂停/停止、开闭环、数据/决策周期、数据/控制时刻、建立/中断连接、当前断面、曲线树和系统曲线。
  2. 设备定义：至少选择风力发电机子页，清楚显示浅黄色可编辑静态字段、灰色实时只读字段和“保存设备修改”。
  3. RTU 定义与连接状态：显示 `operator_io` 连接成功/中断、RTU ID、对端 IP:端口、墙钟刷新时刻、在线数量和 RTU 表。
...
- 第 10 章必须围绕上述截图形成真正的用户操作使用手册，不得只列控件名称。至少给出以下 6 组可执行步骤：顶部控制与连接、系统主页监视、设备参数修改保存、RTU/四遥核对与定义维护、决策日志查询审计、历史多点曲线查询。每组都要说明操作入口、点击/输入顺序、成功判据、时刻格式、错误或风险提示以及自动刷新/编辑保护行为。
