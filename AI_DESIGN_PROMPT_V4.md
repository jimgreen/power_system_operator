...
> 2026-08-25 参数编辑保护与曲线游标更新：顶部开闭环/双周期、设备静态参数及 RTU/四遥定义一旦被人工修改，控件或单元格必须立即使用醒目的橙黄色脏状态提示；保存或用户点击对应区域的“手动刷新参数”并确认放弃修改前，1 秒刷新、`data_period` 周期刷新和切页刷新均不得覆盖未保存值。运行状态、双时钟、连接状态等只读实时量继续刷新，运行/暂停/停止动作只能写 `oper_status`，不得顺带保存待提交参数。系统主页和历史曲线的游标文字框必须跟随鼠标移动、自动避开绘图区边界，并使用完全透明背景。
