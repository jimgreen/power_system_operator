...
from power_operator.runtime_processes import default_io_pid_path, service_pid_file
...
def main() -> None:
    ...
    pid_file = Path(args.pid_file) if args.pid_file else default_io_pid_path(database.path)
    try:
        with service_pid_file(
            pid_file,
