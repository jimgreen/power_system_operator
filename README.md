...
- `operator_history`、`operator_log` 和四张四遥历史表不设置记录数或保留时长上限；除系统归零重启时按既定流程清空外，所有历史记录持续保留。主页曲线和历史曲线仍可访问完整时间范围；运行日志为避免界面堵塞，按过滤结果执行数据库分页，不会一次性加载全部日志，也不会删除或限制数据库中的日志总量。
...
柴油、风机、光伏、储能设备均有 `control_mode`（0 开环、1 闭环，默认 1）。每台设备的模式由实时 YX 属性 2（`设备ID*100+2`，点名“设备名称.控制模式”）更新，只有 `time>0` 才生效；值严格等于 1 为闭环，其他值按开环处理。旧库缺列时补默认 1 并一次性补缺失模式 YX；导入不含该新列的旧定义库时也按 1 兼容。
...
1. 根据四遥点名称更新设备实时功率、SOC、运行状态和设备控制模式。实时量只取 `time > 0` 的 YC/YX；状态按 `<设备名称>.运行状态` 或兼容映射更新，控制模式按 `<设备名称>.控制模式` 或属性 2 点号更新，严格值 1 为闭环、其他值为开环，`time=0` 不覆盖上次模式。多个点映射同一字段时采用时刻最新的一条。已废弃 YC/YT 继续按既有规则过滤，设备字段 `p_max_curr`、`angle_pitch_curr`、YC“当前桨距角”和正常有功设定 YT 不受影响。
...
新能源优先策略先把运行开环设备的实时 `p_curr` 作为固定功率贡献，停机开环设备不计入；EMS 只对 `control_mode=1` 的运行设备调度净需求 `负荷 - 开环固定功率`。闭环设备内部顺序为：
...
- 全局开环：不生成或发送任何设备 YT/YK。
- 全局闭环且设备闭环：每个决策周期写入该设备预定义 YT；YK 仍只在目标状态与实际状态不一致时生成。
- 设备开环：不进入可控调度，其 `p_curr` 作为固定贡献；既有命令时刻清零，Core 审计记 `device_open_loop`，IO 再次过滤，确保遗留 YT/YK 不外发。
...
- 顶部紧凑控制区：运行状态、控制模式、数据周期、决策周期、数据时刻、控制时刻、电网模拟器连接状态、运行操作按钮、“保存参数”“手动刷新参数”以及“建立连接”“中断连接”两个连接按钮；两个时刻均把数据库整数秒显示为 `HH:mm:ss`。连接状态在所有页面始终可见，并按 `scada_rtu.status` 显示绿色“正常”或红色“中断”。
...
- 设备定义：柴油机、风机、光伏、储能和负荷。页面顶部提供“手动刷新参数”和“保存设备修改”；双击浅黄色单元格可修改名称、额定值、上下限、风速阈值、效率、容量及 SOC 上下限等静态参数，人工修改后单元格变为橙黄色且表格显示橙色边框；灰色的设备 ID（已有记录）、状态、实时量和设定值只读。末尾空白行的 ID 和静态参数可用于新增设备。
- RTU / 四遥定义：页面直接提供 RTU、YC、YX、YT、YK 五个表格子页以及“手动刷新参数”“保存 RTU / 四遥修改”按钮，不再重复显示 `operator_io` 连接状态、对端、刷新时刻和在线数量摘要栏；连接状态统一查看所有页面始终可见的顶部“电网模拟器连接”状态卡。人工修改的单元格同样使用橙黄色脏状态提示。
- 运行日志：按日志类型、运行时刻和关键词查询，先统计总数，再使用数据库 `LIMIT/OFFSET` 分页；默认每页 100 条，可选 50/100/200/500 条，并显示总条数、当前页/总页数、上一页和下一页。数据库日志总量不设上限。选择控制决策后，可以分组浏览“触发条件、输入、决策过程、输出、平衡校验/警告”，也可以切换查看保持中文的格式化原始 JSON；旧普通文本日志仍可直接查看。
- 历史曲线：左侧多选四遥点，右侧按时间范围绘制多条曲线。
- 系统主页和历史曲线均支持鼠标数据游标：移动鼠标时吸附到最近的数据时刻，绘制竖向虚线和各曲线数据点；透明背景的文字框跟随鼠标移动，自动在鼠标四周选择不会越出绘图区的位置，并显示 `HH:mm:ss` 时刻及所有已选曲线的三位小数值。鼠标离开、数据切换、缩放或复位后清除旧游标。
...
- 运行日志周期刷新保留当前过滤条件、页码、选中记录、详情页签和滚动位置；新日志使所选记录移动到其他页时会自动定位新页码，用户正在查阅历史决策时不会被强制切换到最新记录。
...
测试覆盖建表与旧库迁移、`power.db` 只读定义导入、决策审计 JSON 和事务回滚、PRAGMA、线程和四进程并发写、调度边界、停止到运行的清理、四遥 `time > 0` 有效性、YK 状态差异判断与发送出口复核、双时钟处理、开闭环命令边界、Bridge 周期与游标、手动连接开关、连接失败状态回写、真实 TCP socket，以及 Qt 无头构造、五个主页面、连接按钮、RTU 页面、运行日志决策详情、运行周期页面刷新、切页立即刷新、橙黄色未保存状态、手动刷新参数、自动刷新编辑保护、历史曲线勾选保护、透明跟随游标和三位小数显示。
