...
- 运行日志：按日志类型、运行时刻和关键词查询，先统计总数，再使用数据库 `LIMIT/OFFSET` 分页；默认每页 100 条，可选 50/100/200/500 条，并显示总条数、当前页/总页数、上一页和下一页。数据库日志总量不设上限。选择控制决策后，顶部“决策过程（逐行日志）”按步骤显示编号、已执行/未触发、中文步骤名、三位小数关键输入输出及中文原因；下方树只保留“触发条件、输入、输出、平衡校验/警告”，原始记录页签继续保留完整中文 JSON；旧普通文本日志仍可直接查看。
...
测试覆盖建表与旧库迁移、`power.db` 只读定义导入、决策审计 JSON 和事务回滚、PRAGMA、线程和四进程并发写、调度边界、停止到运行的清理、四遥 `time > 0` 有效性、YK 状态差异判断与发送出口复核、双时钟处理、开闭环命令边界、Bridge 周期与游标、手动连接开关、连接失败状态回写、真实 TCP socket，以及 Qt 无头构造、五个主页面、连接按钮、RTU 页面、决策过程逐行日志、运行日志按需分页、主页和历史曲线自动刷新、历史点树不周期重建、切页立即刷新、橙黄色未保存状态、手动刷新参数、自动刷新编辑保护、历史曲线勾选保护、透明跟随游标和三位小数显示。
